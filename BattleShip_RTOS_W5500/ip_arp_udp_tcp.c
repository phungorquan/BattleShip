...
const char iphdr[9] ={0x45,0,0,0x82,0,0,0x40,0,0x20}; // 0x82 is the total len on ip, 0x20 is ttl (time to live), the second 0,0 is IP-identification and may be changed.
...
void send_udp_prepare(unsigned char *buf,unsigned int sport, const unsigned char *dip, unsigned int dport,const unsigned char *dstmac)
{
        ...
        //dia chi mac dich, mac nguon 6+6=12 byte
        while(i<6)
				{
						buf[ETH_DST_MAC +i]=dstmac[i]; // gw mac in local lan or host mac PC
						buf[ETH_SRC_MAC +i]=macaddr[i];
                i++;
        }
				//Type 2byte
        ...
        buf[IP_PROTO_P]=IP_PROTO_UDP_V;
				// 4 byte dia chi ip dich va 4 byte ip nguon
        i=0;
        while(i<4)
				{
        ...
        // checksum 2 byte =0 (zero the checksum)
        buf[UDP_CHECKSUM_H_P]=0;
        buf[UDP_CHECKSUM_L_P]=0;
        // copy the data:
