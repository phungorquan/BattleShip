#include "LCDST7920.h" // Get NUC131 and LCD funcs
#include "BattleMap.h" // Get ship funcs
#include "ENC28J60.h"	 // Get Ethernet funcs
#include "ip_arp_udp_tcp.h" // Get UDP protocol funcs
#include "net.h"			// Get some defines of ENC
#include <stdlib.h>		// Avoid abs error
...
#ifdef Board1
// Source
static uint8_t mymac[6] ={0x00,0x1b,0x77,0x71,0x48,0xf1}; 		 // ENC28H60 MAC addr ,must be unique in your network
static uint8_t myip[4] ={192, 168, 0, 116};                    // ENC28H60 IP ,must be unique in your network
// Destination
static uint8_t dis_mac[6] ={0x00,0x1b,0x77,0x71,0x48,0xf0};		 // ENC28H60 MAC addr ,must be unique in your network
static uint8_t dis_ip[4] = {192,168,0,100};                    // ENC28H60 IP ,must be unique in your network
#endif

#ifdef Board2
// Source
static uint8_t mymac[6] ={0x00,0x1b,0x77,0x71,0x48,0xf0}; 		 // ENC28H60 MAC addr ,must be unique in your network
static uint8_t myip[4] ={192, 168, 0, 100};                    // ENC28H60 IP ,must be unique in your network
// Destination
static uint8_t dis_mac[6] ={0x00,0x1b,0x77,0x71,0x48,0xf1};		 // ENC28H60 MAC addr ,must be unique in your network
static uint8_t dis_ip[4] = {192,168,0,116};                    // ENC28H60 IP ,must be unique in your network
#endif

#ifdef PC_Test
// Source
static uint8_t mymac[6] ={0x00,0x1b,0x77,0x71,0x48,0xf0}; 		 // ENC28H60 MAC addr ,must be unique in your network
static uint8_t myip[4] ={192, 168, 1, 116};                    // ENC28H60 IP ,must be unique in your network
// Destination
// Use cmd "ipconfig /all" to find Wifi's MAC or LAN's MAC addr
static uint8_t dis_mac[6] = {0x1C,0x65,0x9D,0x58,0x97,0xF3};   // PC network(LAN or WIFI) MAC addr 
static uint8_t dis_ip[4] = {192,168,1,8};                   	 // PC IP
#endif
...
// X and Y position for whole program
volatile short x = 0;
volatile short y = 0;

// Some defines to define who will be shot first
bool waiting_state = true;
bool passstep = true;
bool My_turn = false;

// Number of blocks (1 , 2x3 , 3x3)
uint8_t Num_of_Blocks_To_Finish = 16;
// Because position in map only from 00 -> 99 , are different from 100 , so i put 100 to compare easily
...
0x00,0x00,0x00,0x00,0x00,0x00,0x40,0x00,0x00,0x02,0x00,0x00,0x00,0x00,0x00,0x00, // 15
0x00,0x00,0x00,0x00,0x00,0x00,0x40,0x00,0x00,0x02,0x00,0x00,0x00,0x00,0x00,0x00, // 31
0x00,0x00,0x00,0x00,0x00,0x00,0x40,0x00,0x00,0x02,0x00,0x00,0x00,0x00,0x00,0x00, // 47
0x00,0x00,0xF8,0xFF,0x1F,0xFC,0x7F,0xFF,0xFF,0xFE,0x3F,0xF8,0xFF,0x1F,0x00,0x00, // 63
		...
		if(Check_Num_of_Blocks_To_Finish[i] == combine_posx_posy_to_decimal) // If this is one of ship position
		{
			for(j = 0 ; j < 16; j++)
			{
				if (Arr_Support_Check_Num_of_Blocks_To_Finish[j] == combine_posx_posy_to_decimal) // If this had existed before
					return true; // return true to display 'BLOCK' for ENEMY instead of 'X' (destroy symbol)
			}
			
			// Following code below avoid COUNTER minus when enemy shoot to one position many times
...
{				
	...
	short _tmpx = 0; // Support avoid user select position too far , only around 1 step  
	short _tmpy = 0; // Support avoid user select position too far , only around 1 step  
	short i = 0;
	short index_ship2 = 0;
	short index_ship3 = 0;
	bool Auto_Set_Ships = true;
	
	Display_Word_SettingShip();	// Display "SETTING SHIP X" and index of selecting ship
	Display_Shoot_XY_Position(_x,_y); // Display position 0 0 for the first time
	...
	while(Ship_Block_Num <= 16)	// 16 block ( Ship1:1block , Ship2: (1block * 2) * 3 , Ship3: (1block * 3) * 3)
		{
			if(LEFTUP == 0)		// Increase X by one
			{
				...
				if(_x == 10)
					_x = 0;
				Display_Shoot_XY_Position(_x,_y);
			}
			...
			{
				while(OK == 0){}
					if(Support_Avoid_Setup_Ship_Asthesame_Positions(_x,_y) == true) // If don't setup as the same position
					{
						if (Ship_Block_Num < 2) // If user are setting up SHIP 1
						{
							Ship1_Positions[0] = _x; // Save x position to Ship1
							Ship1_Positions[1] = _y; // Save y position to Ship1
							
							Check_Num_of_Blocks_To_Finish[0] = _x * 10 + _y;	// Save this position to Arr
							
							Ship_Block_Num++;				// Increase to next block of next ship
							Set_My_Ship_XY(_x,_y);	// Display ship on MAP
							Display_Setup_NumofShip('S',6); // Display ship index selection
							Display_Setup_NumofShip('R',3); // Display ship index selection
						}
						
						else if(Ship_Block_Num > 1 && Ship_Block_Num < 8) // If user are setting up SHIP 2
						{
							// This will check whether user select position too far or not
							// Only effect to the 2nd blocks 
							if(Ship_Block_Num == 2 || Ship_Block_Num == 4 || Ship_Block_Num == 6 )
							{
								_tmpx = _x;
								_tmpy = _y;
								Ship2_Positions[index_ship2] = _x; // Save x position to Ship2
								Ship2_Positions[index_ship2 + 1] = _y; // Save y position to Ship2
								Check_Num_of_Blocks_To_Finish[Ship_Block_Num - 1] = _x * 10 + _y; // Save this position to Arr
								index_ship2 += 2;
							...
							else
							{
								// This will check whether user select position too far or not
								// Only effect to the 2nd blocks 
								if(abs(_x - _tmpx) < 2 && abs(_y - _tmpy) < 2) 
								{
									_tmpx = _x;
									_tmpy = _y;
									Ship2_Positions[index_ship2] = _x; // Save x position to Ship2
								...
								// Display ship index selection
								if(Ship_Block_Num == 4)
									Display_Setup_NumofShip('L',2);
								else if(Ship_Block_Num == 6)
									Display_Setup_NumofShip('L',3);
								else if(Ship_Block_Num == 8)
								{
									Display_Setup_NumofShip('S',7);
									Display_Setup_NumofShip('L',1);
								}
						}
						else // If user are setting up SHIP 3
						{
							// This will check whether user select position too far or not
							// Only effect to the 3rd blocks 
							if(Ship_Block_Num == 8 || Ship_Block_Num == 11 || Ship_Block_Num == 14)
							{
								_tmpx = _x;
								_tmpy = _y;
								
								Ship3_Positions[index_ship3] = _x; // Save x position to Ship3
								Ship3_Positions[index_ship3 + 1] = _y; // Save y position to Ship3
							...
							else
							{
								...
								// Display ship index selection
								if(Ship_Block_Num == 11)
									Display_Setup_NumofShip('L',2);
								else if(Ship_Block_Num == 14)
									Display_Setup_NumofShip('L',3);
								else if(Ship_Block_Num == 17)
									Display_None();
						}
			...
			else if(OK == 0 && Auto_Set_Ships == true)
			{
				...
				Check_Num_of_Blocks_To_Finish[0] = Ship1_Positions[0] * 10 + Ship1_Positions[1]; // Save this position to Arr
				
				//Setup 3Ship2
				for(i=0;i<=10;i=i+2)
					{
						Set_My_Ship_XY(Ship2_Positions[i],Ship2_Positions[i+1]);
						Check_Num_of_Blocks_To_Finish[Ship_Block_Num] = Ship2_Positions[i] * 10 + Ship2_Positions[i+1]; // Save this position to Arr
				...
				for(i=0;i<=16;i=i+2)
					{
						...
						Check_Num_of_Blocks_To_Finish[Ship_Block_Num] = Ship3_Positions[i] * 10 + Ship3_Positions[i+1]; // Save this position to Arr
						Ship_Block_Num++; //Re-use this value to save RAM
					}
				break;
			}
...
{		
    ...
    enc28j60clkout(2); // Change 6.25MHz -> 12.5MHz
		delay(1500);// 
	
		init_ip_arp_udp_tcp(mymac, myip); // Init ENC28J60 MAC and IP
		enc28j60getrev(); // Enable Receive
	
		InitLCD_Graphic();	// Init LCD
		Display_Graphic(BattleMap); // Display for the first time when supply power
...
void vTask1( void *pvParameters )
{
		while(1)
		{
			...
			// Send "waiting" each 4s until receive "waiting" from the other board
...
void vTask2( void *pvParameters )
{
		while(1)
		{
			...
			// If enemy shot, and receive "SHXY" from ENEMY, turn will be switched to ME
			if(My_turn == true)
			{
				Display_Shoot_XY_Position(x,y); // Display for the first time x,y position selection
				while(My_turn == true)
			{
			...
			else if(OK == 0) // After select x,y position to SHOOT , press OK 
			{
				while(OK == 0){}				
				// Send "SHXY" to ENEMY
				sendbuf[UDP_DATA_P]='S';
				sendbuf[UDP_DATA_P+1]='H';
				sendbuf[UDP_DATA_P+2]=x + '0'; // Convert decimal to character
				sendbuf[UDP_DATA_P+3]=y + '0'; // Convert decimal to character
				send_udp_prepare(sendbuf,MySrcPort,dis_ip,DesPort,dis_mac);	
				send_udp_transmit(sendbuf,4);
				Destroy_Enemy_Ship_XY(x,y);		// Display 'X' (Destroy symbol) on the ENEMY screen
				My_turn = false;							// Disable my turn until receive "SHXY" from ENEMY
...
void vTask3( void *pvParameters )
{
    ...
    volatile char datarev[10];
		...
		/***
		* When using RTOS, we CAN NOT use 'continue' function as common.
		* So i will use below method to avoid TASK are stuck in while(1) forever
		* TO GET OLD CODE WITH 'continue', JUST FIND MY BATTLESHIP FOLDER (WHICH DON'T USE RTOS)
		***/ 
				 ...
				 step_1 == true && step_2 == true && step_3 == true && step_4 == true)
      {
				...
				while (cmd_pos < payloadlen)
            {
               ...
               cmd_pos++;
						...
						// If receive "RES1"- hit , "RES0" - missed
						if(datarev[0] == 'R')
						{
							if(datarev[3] == '1')
								Set_Enemy_Ship_XY(x,y); // If hit, display 'BLOCK'
							else if(datarev[3] == '0')
								Destroy_Enemy_Ship_XY(x,y); // If missed, display 'X' (Destroy symbol)
							My_turn = false;					// Disable my turn
							Display_Turn(My_turn);
						}
						...
						{
							uint8_t x_pos_receive = datarev[2] - '0'; // Convert from character to decimal
							uint8_t y_pos_receive = datarev[3] - '0'; // Convert from character to decimal
							if(Find_My_Ship_Position( x_pos_receive,y_pos_receive) == true)	// If ENEMY hit my position
								My_UDP_Str_Send("RES1");	// Response RES1
							else 
								My_UDP_Str_Send("RES0");	// If missed, Response RES0
								
								if(Num_of_Blocks_To_Finish == 0) // If we are shot all of ships
								{
									My_UDP_Str_Send("WIN");	// Send to ENEMY WIN
									
									InitLCD_BasicMode();		// Init BasicMode (Simple characters mode)
									Display_BasicMode("LOSE"); // DISPLAY LOSE
								}
								else // else, destroy my ship at my screen
								{
									Destroy_My_Ship_XY(x_pos_receive,y_pos_receive);
									My_turn = true; // Enable myturn 
									Display_Turn(My_turn);
								}
								
						}
						
						// This procedures to know who will shoot first
						...
						else if(datarev[0] == 'a' && passstep == true)
						{
							...
							Setup_Myship();			// Send "setup" and then setup ship as requirement from lecturer
							My_turn = true;
							Display_Turn(My_turn);
						...
						{
							InitLCD_BasicMode(); // Init BasicMode (Simple characters mode)
							Display_BasicMode("WIN"); // DISPLAY WIN
						}
					
			}
			// Clear buffer, but it maybe not effect :v
			for(i = 0 ; i< 10;i++)
						receivebuf[UDP_DATA_P + i] = '0';
